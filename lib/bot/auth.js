...
/** Cron de plataforma: no confiar solo en el header (se puede spoofear). */
export function cronAuthorized(req) {
  const fromPlatformCron = String(req.headers['x-vercel-cron'] || '') === '1'
    || String(req.headers['x-cloudflare-cron'] || '') === '1';
  if (!fromPlatformCron) return false;
